            ...
            <div id="page-inner">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <h1 class="page-subhead-line">学生管理 </h1>
            ...
            <div class="col-md-12 col-sm-12 col-xs-12">
               <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <form role="form" action="admin/updateStudent" method="post">
                                        <div class="form-group">
                                            <label>学号</label>
                                            <input class="form-control" type="text" name="sno" value="${student.sno}" readonly="readonly">
                                        </div>
										<div class="form-group">
                                            <label>姓名</label>
                                        ...
                                        <div class="form-group">
											<label>专业</label>
											 <select multiple class="form-control" name="smajor" > 
												<c:forEach items="${majorList}" var="major" >
													<option value="${major.smajor}">${major.smajor}</option> 
												</c:forEach>
											  </select>
										</div>
										<div class="form-group">
											<label>年级</label>
											 <select multiple class="form-control" name="sgrade" > 
												<c:forEach items="${gradeList}" var="grade" >
													<option value="${grade.sgrade}">${grade.sgrade}</option> 
												</c:forEach>
											  </select>
										</div>
										<div class="form-group">
											<label>班级</label>
											   <select multiple class="form-control" name="sclass" > 
												<c:forEach items="${classList}" var="sclass" >
													<option value="${sclass.sclass}">${sclass.sclass}</option> 
												</c:forEach>
											  </select>
										</div>
                                        <button type="submit" class="btn btn-info">修改学生</button>
										<button  id="resetpwd"  class="btn btn-info">重置密码</button>
                                </form>
                            </div>
                        </div>
                        </div>
