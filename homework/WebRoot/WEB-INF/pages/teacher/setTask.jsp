			...
			<div style="display:block;clear:both;">
				...
				<div style="width:100%;border:1px solid #edf1f2;background-color:#fff;float:left;margin-top:30px;margin-bottom:50px;">
					<div style="padding:10px 50px 40px;" class="us">
						<p style="font-size:18px;line-height:49px;margin-bottom:2.1em;border-bottom:1px solid #eceff0;color:#363d40;">设置作业</p>
						<!-- 作业内容 -->
						<form action="teacher/addTask/${course.cno}" method="post" enctype="multipart/form-data">
							  <div class="form-group">
							    <label for="exampleInputEmail1">作业标题</label>
							    <input type="text" name="task_title" class="form-control" id="exampleInputEmail1" placeholder="标题">
							  </div>
							  <div class="form-group">
							    <label for="exampleInputPassword1">截止时间</label>
									<input size="16" name="task_stoptime"  data-date-format="yyyy-MM-dd hh:mm"  type="text" value="选择截止时间" readonly class="form_datetime form-control">
							  </div>
							  <div class="form-group">
							    <label for="name">作业内容</label>
							    <textarea class="form-control"  name="task_content" rows="7"></textarea>
							  </div>
							  <div class="form-group">
							    <label for="name">作业附件</label>
								<input type="file" name="homeworkFile" class="form-control">
							  </div>
							  
							  <button type="submit" class="btn btn-primary">添加作业</button>
						</form>
						<hr/>
						<br/>
						  ...
						  <div class="panel panel-default">
						    <div class="panel-heading" role="tab" id="headingOne">
						      <h4 class="panel-title">
						        <a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapse${task.sctimes }" aria-controls="collapseOne">
						          	第${task.sctimes}次作业
						        </a>
						      </h4>
						    </div>
						    <div id="collapse${task.sctimes }" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne">
						      <div class="panel-body">
						      		<form action="teacher/updateTask/${course.cno }" method="post" enctype="multipart/form-data">
										  <input type="hidden" name="task_id" value="${task.task_id}">
										  <div class="form-group">
										    <label for="exampleInputEmail1">作业标题</label>
										    <input type="text" name="task_title" class="form-control" id="exampleInputEmail1" value="${task.task_title}">
										  </div>
										  <div class="form-group">
										    <label for="exampleInputPassword1">截止时间</label>
												<input size="16" name="task_stoptime" type="text"  placeholder="${task.stoptime }"   class="form_datetime form-control">
										  </div>
										  <div class="form-group">
										    <label for="name">作业内容</label>
										    <textarea class="form-control"  name="task_content" rows="7">${task.task_content }</textarea>
										  </div>
										  <div class="form-group">
										    <c:choose>
												<c:when test="${task.task_file!=null}">
													<label for="name">作业附件------<a href="teacher/download/${course.cno }?taskfilename=${task.task_file}" style="color:red;font-size:10px;">${task.task_file}</a></label>
												</c:when>
												<c:otherwise>
													<label for="name">作业附件</label>
												</c:otherwise>
